...
class Mount(interfaces.plugins.PluginInterface):
    ...
    def get_all_mounts(
        ...
        for hash in mount_hashtable:
            ...
            for mount in first_mount.mnt_hash.to_list(
                symbol_table + constants.BANG + mnt_type,
                "mnt_hash",
                sentinel=False,
            ):
                ...
                try:
                    ...
                    if (
                        not devname
                        or not devname.isprintable()
                        or not all(ord(c) < 128 for c in devname)
                    ):
                        break
                ...
                try:
                    ...
                    if (
                        not fs_type
                        or not fs_type.isprintable()
                        or not all(ord(c) < 128 for c in fs_type)
                    ):
                        break
    ...
    def get_mounts(
        ...
        for task in pslist.PsList.list_tasks(
            ...
            # walk mount list
            for mount in mnt_ns.list.to_list(
                symbol_table + constants.BANG + mnt_type, "mnt_list"
            ):
                yield task, mount
        ...
        if task is not None:
            try:
                path = symbols.linux.LinuxUtilities.prepend_path(
                    mount.get_mnt_root().dereference(),
                    mount,
                    task.fs.root,
                )
            except exceptions.PagedInvalidAddressException:
    ...
    def _generator(self):
        ...
        if self.config.get("sort", False):
            mounts_by_id = {
                mount.mnt_id: (task, mount) for task, mount in mounts
            }
